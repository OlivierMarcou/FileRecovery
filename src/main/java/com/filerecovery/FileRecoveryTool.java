...
public class FileRecoveryTool extends JFrame {
    ...
    private JPanel createTopPanel() {
        ...
        panel.setBorder(BorderFactory.createTitledBorder("Sélection du périphérique / lecteur"));
        ...
        unmountButton = new JButton("Démonter / Éjecter");
        ...
        rawAccessCheckbox = new JCheckBox("Accès direct (Linux root requis)", true);
    ...
    private void startScan() {
        ...
        if (device.isMounted && !device.path.equals("/dev/null") && !device.path.equals("NULL")) {
            if (!isWindows) {
                ...
                JOptionPane.showMessageDialog(this,
                        "Le lecteur " + device.path + " est actuellement monté.\n" +
                                "Pour un scan complet, veuillez le démonter d'abord.\n\n" +
                                "Le scan continuera en mode lecture standard.",
            ...
            protected Void doInBackground() {
                try {
                    if (device.path.equals("/dev/null") || device.path.equals("NULL")) {
                        ...
                        if (rawAccessCheckbox.isSelected() && !isWindows) {
                            publish("Scan en mode accès direct (raw)...");
                            scanDeviceRaw(device);
                        } else if (rawAccessCheckbox.isSelected() && isWindows) {
                            publish("Mode raw non disponible sous Windows sans driver spécial...");
                            publish("Utilisation du mode fichier standard...");
                            scanDeviceFile(device);
                        } else {
                            publish("Scan en mode fichier...");
                            scanDeviceFile(device);
                        }
                    }
                } catch (Exception e) {
    ...
    private void scanDeviceRaw(BlockDevice device) throws IOException {
        ...
        if (!devFile.canRead()) {
            throw new IOException("Permissions insuffisantes. Exécutez avec sudo.");
        }
        ...
        try (RandomAccessFile raf = new RandomAccessFile(device.path, "r");
            ...
            while (position < deviceSize && position < 100 * 1024 * 1024) {
                ...
                if (sectorsScanned % 100 == 0) {
                    int progress = (int)((position * 100) / Math.min(deviceSize, 100 * 1024 * 1024));
                    logStatus("Progression: " + progress + "% (" + formatSize(position) + " scannés)");
                }
            }

            logStatus("✓ Scan de signatures terminé");
        }
    }
